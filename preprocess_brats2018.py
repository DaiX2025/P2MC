...
def crop(vol):
    ...
    x_min, x_max = sup_128(x_min, x_max, b=240)
    y_min, y_max = sup_128(y_min, y_max, b=240)
    z_min, z_max = sup_128(z_min, z_max, b=155)
...
for file_name in name_list:
    ...
    print(vol.shape)
    if vol.shape != (4,240,240,155):
        print(f"{file_name} wrong shape: {vol.shape}")
        exit(0)
    vol1 =vol[:, x_min:x_max, y_min:y_max, z_min:z_max]
    vol1 = normalize(vol1)
    print(np.min(vol1), np.max(vol1))
    print(vol1.shape)
    vol1 = vol1.transpose(1,2,3,0)
    print(vol1.shape)
    if min(vol1[:,:,:,0].shape) < 128:
        print(f"{file_name} wrong resized shape: {vol1.shape}")
        exit(0)
