...
def confusion_matrix(x, y, n, ignore_label=None, mask=None):
    ...
    k = (x_flat >= 0) & (y_flat < n) & (x_flat != ignore_label) & (mask_flat.astype(np.bool_))
    
    conf_mat = np.bincount(n * x_flat[k].astype(int) + y_flat[k], minlength=n ** 2).reshape(n, n)
...
def getScores(conf_matrix):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        # Dice coefficient
        dice = 2 * np.diag(conf_matrix) / (conf_matrix.sum(1) + conf_matrix.sum(0)).astype(float)
    
    return overall * 100., np.nanmean(perclass) * 100., np.nanmean(IU) * 100., np.nanmean(dice) * 100.
...
class Saver():
    ...
    def save(self, new_val, dict_to_save):
        ...
        if self._do_save(new_val):
            ...
            dict_to_save['best_val'] = new_val
            torch.save(dict_to_save, '{}/model-last.pth.tar'.format(self.ckpt_dir))
