...
class Rescale3D(Base):
    def __init__(self, output_size):
        assert isinstance(output_size, (tuple, list)) and len(output_size) == 3
        self.output_size = output_size

    def sample(self, *shape):
        self.zoom_factors = [o / float(s) for o, s in zip(self.output_size, shape)]
        return self.output_size

    def tf(self, img, k=0):
        # Set the interpolation order based on whether k is 0 or 1
        order = 3 if k == 0 else 0  # Use order=3 for images, order=0 for masks
        batch_size = img.shape[0]
        
        # Initialize an array to hold the rescaled output
        rescaled = np.zeros((batch_size, *self.output_size) + (() if k == 1 else (img.shape[-1],)), dtype=img.dtype)

        for bs in range(batch_size):
            if k == 0:
                # Rescale each channel of the image separately
                channels = [zoom(img[bs, :, :, :, c], self.zoom_factors, order=order, mode='constant', cval=0) 
                            for c in range(img.shape[-1])]
                rescaled[bs] = np.stack(channels, axis=-1)
            else:
                # Rescale the mask
                rescaled[bs] = zoom(img[bs], self.zoom_factors, order=order, mode='constant', cval=0)
...
class RandCrop3D(CenterCrop):
    def sample(self, *shape):  # shape: [240,240,155]
        ...
        start = []
        for i, s in zip(size, shape):
            if s > i:
                start.append(random.randint(0, s - i))
            else:
                start.append(0)
                # print(f"Warning: Crop size {i} exceeds image dimension {s}, setting start to 0")
